...
from nerf.raymarcher import EmissionAbsorptionNeRFFrontToBackRaymarcher
...
class PixelNeRFFrontToBackRenderer(NeRFRenderer):
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            density_noise_std,
            # scene_encoder,
            # use_image_feats,  # type: ignore
            # transform_to_source_view,
            # resnetfc,
            # ignore_index,
            # use_depth,
            # use_view_dirs,
        ...
        for render_pass in ("coarse", "fine"):
            if render_pass == "coarse":
                ...
                if not sample_sphere:
                    raysampler = PixelNeRFRaysampler(
            ...
            self._implicit_function[render_pass] = PixelNeRF(
                ...
                append_xyz=append_xyz,  # type: ignore
                image_feature_dim=scene_encoder.latent_size if scene_encoder is not None else 0,
    ...
    def forward(
        ...
        chunk_outputs = [
            self._process_ray_chunk(
                ...
                depth,
                source_camera,
                source_image_feats,  # type: ignore
                chunk_idx,
            )
        ...
        if image is not None:
            for render_pass in ("coarse", "fine"):
                ...
                # depth loss
                if self._use_depth:
                    invalid_pixels = torch.logical_or(torch.isnan(
                        out['depth_gt']), out['depth_gt'] > 20.0)
                    valid_mask = torch.logical_not(invalid_pixels)
                    loss_depth = (
                        out["depth_" + render_pass][valid_mask] - out["depth_gt"][valid_mask]) ** 2
    ...
    def forward_mesh(
        ...
        out['rays_points'] = torch.cat(
            [ray_bundle_to_ray_points(ch_o['ray_bundle'])
             for ch_o in chunk_outputs],
            dim=1,
